...
import { useDataTableStore, useDataTableStoreOrUndefined } from '@/providers/dataTable';
...
const TableComponentFactory: React.FC<{ model: ITableComponentProps; columnsMismatch: boolean }> = ({ model, columnsMismatch }) => {
  const store = useDataTableStore();
  const {
    refreshTable, exportToExcel, changeQuickSearch, performQuickSearch, toggleAdvancedFilter, setRowData, onGroup: group, onSort: sort, changeSelectedIds, clearSelectedRow, setCurrentPage, changePageSize,
  } = store;

  useComponentApi<DataTableApi>({ model, typeName: 'DataTableApi',
    typeDefinition: { typeName: 'DataTableApi', files: [{ content: apiCode, fileName: 'apis/dataTableApi.ts' }] },
    api: { refreshTable, exportToExcel, changeQuickSearch, performQuickSearch, toggleAdvancedFilter, setRowData, group, sort, changeSelectedIds, clearSelectedRow, setCurrentPage, changePageSize },
    properties: [
      { name: 'tableData', getter: () => store.tableData },
      ...
      { name: 'totalRows', getter: () => store.totalRows },
      { name: 'totalRowsBeforeFilter', getter: () => store.totalRowsBeforeFilter },
      { name: 'quickSearch', getter: () => store.quickSearch },
      { name: 'userSorting', getter: () => store.userSorting },
      { name: 'grouping', getter: () => store.grouping },
      { name: 'tableFilter', getter: () => store.tableFilter },
      { name: 'selectedStoredFilterIds', getter: () => store.selectedStoredFilterIds },
      { name: 'selectedRow', getter: () => store.selectedRow },
      { name: 'selectedIds', getter: () => store.selectedIds },
      { name: 'isFetchingTableData', getter: () => store.isFetchingTableData },
      { name: 'selectedRows', getter: () => store.selectedRows },
    ],
  }, [store]);
...
const TableComponent: TableComponentDefinition = {
  ...
  Factory: ({ model }) => {
    ...
    const hasStore = isDefined(store);
    ...
    const configuredColumns = useMemo(() => flattenConfiguredColumns(model.items as ColumnsItemProps[]), [model.items]);
    const metadataProperties = useMemo(() => (metadata && isPropertiesArray(metadata.metadata?.properties) ? metadata.metadata.properties : []), [metadata]);
    const metadataPropertyNameSet = useMemo(() => new Set(collectMetadataPropertyPaths(metadataProperties)), [metadataProperties]);
    const dataColumns = useMemo(() => configuredColumns.filter((column) => column.columnType === 'data'), [configuredColumns]);
    ...
    useComponentValidation(() => {
      ...
      if (columnsMismatch)
        errors.push({ propertyName: 'Column Mismatches', error: columnsMismatchError });

      // Return validation result if there are errors
      if (errors.length > 0)
        return { hasErrors: true, validationType: 'error' as const, errors };

      return undefined;
    }, [store, store?.fetchTableDataError, columnsMismatch]);

    return model.hidden === true ? null : store
      ? <TableComponentFactory model={model} columnsMismatch={columnsMismatch} />
  ...
  migrator: (m) => m
    ...
    .add<ITableComponentProps>(27, (prev) => ({
      ...prev, hoverHighlight: true,
      mobile: { ...prev.mobile, hoverHighlight: true },
      tablet: { ...prev.tablet, hoverHighlight: true },
      desktop: { ...prev.desktop, hoverHighlight: true },
    }))
